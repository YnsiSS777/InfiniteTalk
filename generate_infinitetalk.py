...
from PIL import Image  # noqa: F401 (conservé si besoin)

# --- PATCH: rendez le repo importable même si le worker n'a pas hérité du PYTHONPATH
ROOT = "/workspace/InfiniteTalk"
sys.path.insert(0, ROOT)
sys.path.insert(0, os.path.join(ROOT, "src"))

# --- PATCH: neutraliser les kernels exotiques au 1er run (on réactivera plus tard si besoin)
os.environ.setdefault("WAN_FORCE_PYTORCH_ATTENTION", "1")
os.environ.setdefault("WAN_USE_FLASH_ATTN", "0")
os.environ.setdefault("WAN_USE_XFORMERS", "0")
os.environ.setdefault("DISABLE_FLASH_ATTN", "1")
# Si build CUDA à la volée: limiter aux archs Ada (4090/50xx). À adapter selon GPU réel.
os.environ.setdefault("TORCH_CUDA_ARCH_LIST", "8.9")

# Imports du pipeline et utilitaires
import wan
...
# --- PATCH: import Kokoro uniquement si nécessaire (audio_mode == 'tts')
def _lazy_import_kokoro():
    global KPipeline
    from kokoro import KPipeline  # import local (lazy)
...
def _parse_args():
    ...
    parser.add_argument("--task", type=str, default="infinitetalk-14B",
                        choices=list(WAN_CONFIGS.keys()), help="The task to run.")
    parser.add_argument("--size", type=str, default="infinitetalk-480",
                        choices=list(SIZE_CONFIGS.keys()),
                        help=("The buckget size of the generated video. "
    ...
    parser.add_argument("--audio_save_dir", type=str, default='save_audio',
                        help="The path to save the audio embedding.")
    parser.add_argument("--base_seed", type=int, default=42,
                        help="The seed to use for generating the image or video.")
    parser.add_argument("--input_json", type=str, default='examples.json',
    ...
    parser.add_argument("--num_persistent_param_in_dit", type=int, default=None, required=False,
...
# --- PATCH: compat JSON “léger” de ton API
def _coerce_minimal_input_json(path):
    """
    Supporte le JSON 'léger' produit par l'API:
      [
        {
          "input_type": "image" | "video",
          "input_path": "/workspace/uploads/xxx_image.png|xxx_video.mp4",
          "audio_path": "/workspace/uploads/xxx_audio.wav",
          "output_path": "/workspace/outputs/xxx_output.mp4"
        }
      ]
    Renvoie (input_data_dict, output_path or None)
    """
    ...
    if isinstance(data, list) and data and isinstance(data[0], dict):
        item = data[0]
        cond_video = item.get("input_path")
...
def audio_prepare_multi(left_path, right_path, audio_type, sample_rate=16000):
    ...
    if audio_type == 'para':
        ...
        new_human_speech1 = np.concatenate([human_speech_array1[:human_speech_array1.shape[0]],
                                            np.zeros(human_speech_array2.shape[0])])
        new_human_speech2 = np.concatenate([np.zeros(human_speech_array1.shape[0]),
...
def process_tts_multi(text, save_dir, voice1, voice2):
    ...
    for _, (speaker, content) in enumerate(matches):
        if speaker == '1':
            voice_tensor = torch.load(voice1, weights_only=True)
...
def generate(args):
    ...
    if world_size > 1:
        ...
        assert not (args.t5_fsdp or args.dit_fsdp), "t5_fsdp and dit_fsdp are not supported in non-distributed."
        assert not (args.ulysses_size > 1 or args.ring_size > 1), "context parallel is not supported in non-distributed."

    if args.ulysses_size > 1 or args.ring_size > 1:
        assert args.ulysses_size * args.ring_size == world_size, (
            "`ulysses_size * ring_size` must equal the world size."
        )
    ...
    if args.ulysses_size > 1:
        assert cfg.num_heads % args.ulysses_size == 0, "`cfg.num_heads` must be divisible by `ulysses_size`."
    ...
    wan_i2v = wan.InfiniteTalkPipeline(
        ...
        device_id=(local_rank if torch.cuda.is_available() else -1),
        rank=rank,
    ...
    # --- PATCH: support du JSON minimal
    input_data, out_from_minimal = _coerce_minimal_input_json(args.input_json)

    # initialisation audio encoder
    wav2vec_feature_extractor, audio_encoder = custom_init('cpu', args.wav2vec_dir)

    # nom de dossier audio embeddings
    cond_video_name = os.path.basename(input_data['cond_video']).split('.')[0]
    args.audio_save_dir = os.path.join(args.audio_save_dir, cond_video_name)
    os.makedirs(args.audio_save_dir, exist_ok=True)

    conds_list = []

    # segmentation de scènes (si vidéo) sinon chemin direct
    ...
    # mix audio global pour mux final
    if len(input_data['cond_audio']) == 2:
        new_human_speech1, new_human_speech2, sum_human_speechs = audio_prepare_multi(
            ...
            input_data.get('audio_type', 'para')
        )
    ...
    for idx, items in enumerate(zip(*conds_list)):
        ...
        if args.audio_mode == 'localfile':
            if len(input_data['cond_audio']) == 2:
                new_human_speech1, new_human_speech2, sum_human_speechs = audio_prepare_multi(
                    items[1], items[2], input_data.get('audio_type', 'para')
                )
                ...
                v_length = audio_embedding_1.shape[0] if audio_embedding_1 is not None else 0
            else:
                ...
                input_clip['video_audio'] = sum_audio
                v_length = audio_embedding.shape[0] if audio_embedding is not None else 0

        elif args.audio_mode == 'tts':
            # Si tu veux du TTS, adapte ici selon ton schéma d'entrée.
            raise NotImplementedError("audio_mode='tts' not configured for your JSON format.")
    ...
    if rank == 0:
        if args.save_file is None and out_from_minimal:
            # enlever .mp4, save_video_ffmpeg rajoute l'extension
            args.save_file = os.path.splitext(out_from_minimal)[0]
        elif args.save_file is None:
            formatted_time = datetime.now().strftime("%Y%m%d_%H%M%S")
            formatted_prompt = input_clip.get('prompt', '').replace(" ", "_").replace("/", "_")[:50]
            args.save_file = (
                f"{args.task}_{args.size.replace('*','x') if sys.platform=='win32' else args.size}_"
                f"{args.ulysses_size}_{args.ring_size}_{formatted_prompt}_{formatted_time}"
            )
